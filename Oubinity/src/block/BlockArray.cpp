...
#include <cstring>
namespace Block {
BlockArray::BlockArray(LevelOfDetail::LevelOfDetail lod) : m_lod{lod} {
  int chunk_size_cubed =
      m_lod.block_amount * m_lod.block_amount * m_lod.block_amount;
  block_array = new block_id[chunk_size_cubed]();
}
BlockArray::~BlockArray() { delete block_array; }
...
int BlockArray::getBlockIndex(glm::ivec3 block_pos) const {
  ...
  return block_pos.x * chunk_size * chunk_size + block_pos.y * chunk_size +
         block_pos.z;
}
