...
Shader::Shader(std::string vertex_path, std::string fragment_path) {
  ...
  unsigned int fragment_shader =
      compileShader(GL_FRAGMENT_SHADER, code.fragment);
...
void Shader::setUniformVec3i(const std::string& name,
                             const glm::ivec3& vec) const {
  glUniform3iv(glGetUniformLocation(m_id, name.c_str()), 1,
               glm::value_ptr(vec));
}

void Shader::setUniformVec2i(const std::string& name,
                             const glm::ivec2& vec) const {
  glUniform2iv(glGetUniformLocation(m_id, name.c_str()), 1,
               glm::value_ptr(vec));
}

void Shader::setUniformVec2f(const std::string& name,
...
shader_code Shader::parseShader(std::string vertex_path,
  ...
  try {
    ...
    std::cerr << "Error: Shader file not succesfully read: " << e.what()
              << std::endl;
    return shader_code{"", ""};
  }
}
...
void Shader::createShaderProgram(unsigned int vertex_shader,
                                 unsigned int fragment_shader) {
...
void Shader::checkCompileErrors(unsigned int shader_id,
                                int program_type) const {
  int success;
  char infoLog[1024];
  ...
  if (GL_VERTEX_SHADER == program_type) {
    ...
    if (!success) {
      ...
      std::cerr
          << "ERROR::VERTEX_SHADER_COMPILATION_ERROR: "
          << "\n"
          << infoLog
          << "\n -- --------------------------------------------------- -- "
          << std::endl;
    ...
    if (!success) {
      std::cerr
          << "ERROR::FRAGMENT_SHADER_COMPILATION_ERROR: "
          << "\n"
          << infoLog
          << "\n -- --------------------------------------------------- -- "
          << std::endl;
