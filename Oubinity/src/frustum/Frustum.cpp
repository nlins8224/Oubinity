...
void Frustum::update() {
  ...
  m_planes[Planes::BOTTOM].A =
      m_view_projection[0][3] + m_view_projection[0][1];
  m_planes[Planes::BOTTOM].B =
      m_view_projection[1][3] + m_view_projection[1][1];
  m_planes[Planes::BOTTOM].C =
      m_view_projection[2][3] + m_view_projection[2][1];
  m_planes[Planes::BOTTOM].D =
      m_view_projection[3][3] + m_view_projection[3][1];
  ...
  for (Plane plane : m_planes) plane.normalize();
}

bool Frustum::isPointInFrustum(const glm::vec3& world_point) const {
  for (const Plane plane : m_planes) {
    if (plane.distanceToPoint(world_point) < 0) return false;
  }
  return true;
}
