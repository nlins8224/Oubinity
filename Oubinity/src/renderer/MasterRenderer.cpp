...
MasterRenderer::MasterRenderer(TerrainGenerator& terrain_generator,
                               Camera& camera, GLuint skybox_texture_id,
                               GLuint texture_array_id, GLuint water_texture_id,
                               float water_height, int water_area,
                               GLuint cloud_noise_texture_id,
                               glm::ivec2 resolution)
    : m_chunk_renderer{ChunkRenderer(terrain_generator, ChunkShader(), camera,
                                     texture_array_id)},
      m_skybox_renderer{SkyboxShader(), skybox_texture_id},
      m_water_renderer{WaterShader(), water_texture_id, water_height,
                       water_area},
...
ChunkRenderer& MasterRenderer::getChunkRenderer() { return m_chunk_renderer; }
...
WaterRenderer& MasterRenderer::getWaterRenderer() { return m_water_renderer; }

SkyRenderer& MasterRenderer::getSkyRenderer() { return m_sky_renderer; }
