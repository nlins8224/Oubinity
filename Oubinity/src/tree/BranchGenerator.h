...
struct AttractionPoint {
  ...
  bool operator==(const AttractionPoint& other) const {
    if (this->pos == other.pos && this->reached == other.reached &&
...
struct Node {
  ...
  std::unordered_set<AttractionPoint, AttractionPoint::HashFunction>
      attraction_points_in_range;
  Node* parent;
  std::vector<Node*> childs;

  Node(glm::vec3 o_pos,
       std::unordered_set<AttractionPoint, AttractionPoint::HashFunction> o_ap,
       Node* o_parent, std::vector<Node*> o_childs)
      : pos{o_pos},
...
struct Branch {
  ...
  Branch(Node* o_v, Node* o_u) : v{o_v}, u{o_u} {}
