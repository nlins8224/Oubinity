...
#pragma warning( \
    disable : 4868)  // compiler may not enforce left-to-right evaluation order
...
struct GroupPortableImpl {
  ...
  BitMask<uint64_t, kWidth, 3> MatchEmptyOrDeleted()
      const {  // lsb of each byte is 0 for empty or deleted
    constexpr uint64_t msbs = 0x8080808080808080ULL;
  ...
  PHMAP_ATTRIBUTE_REINITIALIZES void clear() {
    ...
    if (capacity_) {
      PHMAP_IF_CONSTEXPR(
        ...
        // node map or not trivially destructible... we  need to iterate and
        // destroy values one by one
  ...
  void destroy_slots() {
    ...
    PHMAP_IF_CONSTEXPR(
        (!std::is_trivially_destructible<
         ...
         std::is_same<typename Policy::is_flat, std::false_type>::value)) {
      // node map, or not trivially destructible... we  need to iterate and
      // destroy values one by one std::cout << "either this is a node map or "
      // << type_name<typename PolicyTraits::value_type>()  << " is not
...
class parallel_hash_set {
 ...
 public:
  ...
  std::pair<iterator, bool> make_rv(
      Inner* inner, const std::pair<EmbeddedIterator, bool>& res) {
  ...
  // value (under write lock protection) and can update the mapped value. if map
  // does not contains key, the second lambda is called and it should invoke the
 ...
 public:
  ...
  template <class K = key_type, class... Args,
            typename std::enable_if<
                !std::is_convertible<K, const_iterator>::value, int>::type = 0,
            K* = nullptr>
  std::pair<iterator, bool> try_emplace_with_hash(size_t hashval,
                                                  key_arg<K>&& k,
                                                  Args&&... args) {
...
template <class T, class Hash, class Eq, class Alloc, size_t N, class Mtx_,
          class Pred>
std::size_t erase_if(
    phmap::parallel_flat_hash_set<T, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
  return phmap::priv::erase_if(c, std::move(pred));
}
...
template <class T, class Hash, class Eq, class Alloc, size_t N, class Mtx_,
          class Pred>
std::size_t erase_if(
    phmap::parallel_node_hash_set<T, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
  return phmap::priv::erase_if(c, std::move(pred));
}
...
std::size_t erase_if(
    phmap::parallel_flat_hash_map<K, V, Hash, Eq, Alloc, N, Mtx_>& c,
    Pred pred) {
  return phmap::priv::erase_if(c, std::move(pred));
}
...
template <class K, class V, class Hash, class Eq, class Alloc, size_t N,
          class Mtx_, class Pred>
std::size_t erase_if(
    phmap::parallel_node_hash_map<K, V, Hash, Eq, Alloc, N, Mtx_>& c,
    Pred pred) {
  return phmap::priv::erase_if(c, std::move(pred));
}
