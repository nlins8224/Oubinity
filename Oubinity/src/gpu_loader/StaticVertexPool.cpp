...
//        LOG_F(INFO, "Empty chunk chunk at pos (%d, %d, %d), no faces added",
//        chunk_pos.x, chunk_pos.y, chunk_pos.z); return;
//    }
//
...
//        1,
//        first_free_bucket->_start_offset, // offset in
...
//    glm::ivec3 chunk_pos = chunk.getPos();
//    m_stats.max_vertices_occurred = std::max(m_stats.max_vertices_occurred,
//    (size_t)added_faces * 6); m_stats.min_vertices_occurred =
//    std::min(m_stats.min_vertices_occurred, (size_t)added_faces * 6);
//    LOG_F(INFO, "max verts: %d, min verts: %d", m_stats.max_vertices_occurred,
//    m_stats.min_vertices_occurred); std::vector<Vertex> mesh{
//    chunk.getMesh().getMeshDataCopy() }; size_t id = first_free_bucket->_id;
//    //LOG_F(INFO, "First free bucket id: %d, start offset: %d, chunk pos: (%d,
//    %d, %d)", id, first_free_bucket->_start_offset, chunk_pos.x, chunk_pos.y,
//    chunk_pos.z);
//
...
//    chunk.getWorldPos(), id };
//    m_chunk_metadata.active_chunks_lod.chunks_lod[daic_id] =
//    static_cast<GLuint>(lod.block_size); m_chunk_pos_to_bucket_id[chunk_pos] =
//    id; m_bucket_id_to_daic_id[first_free_bucket->_id] = daic_id;
//
...
//        LOG_F(WARNING, "Chunk at (%d, %d, %d) not found", chunk_pos.x,
//        chunk_pos.y, chunk_pos.z); return;
//    }
//    fastErase(chunk_pos);
//}
//
...
//    size_t bucket_id_of_last_daic =
//    getBucketIdFromStartOffset(m_chunk_metadata.active_daics[last_daic_id].first);
//
//    LOG_F(INFO, "daic_id: %zu, bucket_id: %zu, last_daic_id: %zu,
//    last_bucket_id: %zu, chunk_pos: (%d, %d, %d)",
//        daic_id, bucket_id, last_daic_id, bucket_id_of_last_daic, chunk_pos.x,
//        chunk_pos.y, chunk_pos.z);
//
//    if (m_chunk_metadata.active_daics.empty() || last_daic_id < daic_id) {
//        LOG_F(ERROR, "An attempt to delete element %d was made, but DAIC size
//        is %d", daic_id, last_daic_id); return;
//    }
//
//    m_chunk_metadata.active_chunk_info.chunk_pos[daic_id] =
...
//        while (1)
//        {
//            GLenum waitReturn = glClientWaitSync(m_sync,
//            GL_SYNC_FLUSH_COMMANDS_BIT, 1); if (waitReturn ==
//            GL_ALREADY_SIGNALED || waitReturn == GL_CONDITION_SATISFIED) {
//                return;
//            }
//        }
//    }
...
//{
//    for (size_t bucket_idx = 0; bucket_idx < DebugVertexPool::MAX_DAIC_AMOUNT;
//    bucket_idx++)
//    {
//        chunk_buckets[bucket_idx]._start_ptr = m_mesh_persistent_buffer +
//        (DebugVertexPool::MAX_VERTICES_IN_BUCKET * bucket_idx);
//        chunk_buckets[bucket_idx]._start_offset =
//        DebugVertexPool::MAX_VERTICES_IN_BUCKET * bucket_idx;
...
//        GL_TRIANGLES,
//        (GLvoid*)0,                                   // start with first draw
//        command m_chunk_metadata.active_daics.size(),     // DAIC amount 0 //
//        tightly packed
//    );
//}
//
...
//    GL_MAP_COHERENT_BIT; size_t buffer_size = DebugVertexPool::MAX_DAIC_AMOUNT
//    * DebugVertexPool::MAX_VERTICES_IN_BUCKET * sizeof(Vertex); LOG_F(INFO,
//    "createMeshBuffer buffer_size: %zu, daic size: %d", buffer_size,
//    m_chunk_metadata.active_daics.size()); glBindBuffer(GL_ARRAY_BUFFER,
//    m_vbo); glBufferStorage(GL_ARRAY_BUFFER, buffer_size, 0, flags);
//    m_mesh_persistent_buffer = (Vertex*)glMapBufferRange(GL_ARRAY_BUFFER, 0,
//    buffer_size, flags);
//
...
//
//    glBindBuffer(GL_DRAW_INDIRECT_BUFFER, m_daicbo);
//    glBufferData(GL_DRAW_INDIRECT_BUFFER, m_chunk_metadata.active_daics.size()
//    * sizeof(DAIC), NULL, GL_DYNAMIC_DRAW);
//}
//
// void StaticVertexPool::updateMeshBuffer(std::vector<Vertex>& mesh, int
// buffer_offset)
//{
//    OPTICK_EVENT("updateMeshBuffer");
//    waitBuffer();
//    std::copy(mesh.begin(), mesh.end(), m_mesh_persistent_buffer +
//    buffer_offset); lockBuffer();
//
//    //int index = 0;
//    //for (DAIC daic : m_chunk_metadata.active_daics) {
//    //    LOG_F(INFO, "DAIC: %d, count: %d, first: %d", index, daic.count,
//    daic.first);
//    //    index++;
//    //}
//
//    glBindBuffer(GL_DRAW_INDIRECT_BUFFER, m_daicbo);
//    glBufferData(GL_DRAW_INDIRECT_BUFFER, m_chunk_metadata.active_daics.size()
//    * sizeof(DAIC), m_chunk_metadata.active_daics.data(), GL_DYNAMIC_DRAW);
//}
//
