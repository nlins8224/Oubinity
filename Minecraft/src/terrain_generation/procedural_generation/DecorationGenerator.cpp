...
void DecorationGenerator::generateTrees(Chunk& chunk, HeightMap& surface_map, TreePresenceMap& tree_presence_map, uint8_t water_height)
{
	...
	for (uint8_t x = 0; x < lod.block_amount; x++)
	{
		for (uint8_t z = 0; z < lod.block_amount; z++)
		{
			...
			{
				uint8_t trunk_height = m_tree_shape.trunk_height + ((z + x) % 4u);
				uint8_t crown_height = m_tree_shape.crown_height + ((z + x) % 4u);
				uint8_t crown_width = m_tree_shape.crown_width + ((z + x) % 4u);
				crown_height += crown_height % 2 == 0; // round to odd
				crown_width += crown_width % 2 == 0; // round to odd

				Tree tree{ trunk_height, crown_height, crown_width };
				uint8_t tree_plant_height = static_cast<uint8_t>(surface_map[x][z]) % lod.block_amount;
