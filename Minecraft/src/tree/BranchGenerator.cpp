...
std::vector<ProceduralTree::Branch> ProceduralTree::BranchGenerator::getLastGeneratedBranches()
{
	return m_branches;
}

void ProceduralTree::BranchGenerator::generateAttractionPoints(glm::ivec3 tree_pos)
{
	...
	for (size_t x = 0; x < CHUNK_SIZE; x += 8)
	{
		for (size_t y = 0; y < CHUNK_SIZE; y += 8)
		{
			...
			{
				glm::vec3 ap_pos = tree_pos + glm::ivec3( dist(generator_x), dist_unsigned(generator_y) + range, dist(generator_z) );
...
std::vector<ProceduralTree::Branch> ProceduralTree::BranchGenerator::generateBranches(glm::ivec3 tree_pos)
{
	generateAttractionPoints(tree_pos);
	createRootNode(tree_pos);
	size_t iter_count = 0;
	for (size_t i = 0; i < m_settings.max_iterations && atLeastOneAttractionPointExists() && nodesAreInAttractionPointsRange(); i++)
	{
		doIteration();
	}
	return m_branches;
...
{
	Node* root = new Node(tree_pos, std::unordered_set<AttractionPoint, AttractionPoint::HashFunction>(), nullptr, std::vector<Node*>());
...
void ProceduralTree::BranchGenerator::createNewNode(Node* parent)
{
	...
	glm::vec3 scaled_direction = { direction.x * m_settings.branch_length, direction.y * m_settings.branch_length, direction.z * m_settings.branch_length };
	...
	Node* child = new Node( new_node_pos, std::unordered_set<AttractionPoint, AttractionPoint::HashFunction>(), parent, std::vector<Node*>() );
...
{
	m_attraction_points.erase(std::remove_if(begin(m_attraction_points), end(m_attraction_points),
		[](AttractionPoint const& ap) {
			return ap.reached == true;
		}),
		m_attraction_points.end());
}

uint32_t ProceduralTree::BranchGenerator::calculateDistance(AttractionPoint ap, Node node)
{
	return glm::length(calculateDistanceVec(ap, node));
}

glm::vec3 ProceduralTree::BranchGenerator::calculateDistanceVec(AttractionPoint ap, Node node)
{
...
void ProceduralTree::BranchGenerator::printBranches()
{
	for (auto& branch : m_branches)
	{
		...
		LOG_F(INFO, "branch  pos: (%f, %f, %f) -> (%f, %f, %f)", v.x, v.y, v.z, u.x, u.y, u.z);;
	}
}
